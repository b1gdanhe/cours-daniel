...
function int(string $name, $value, int $min = 0,  $max = INF): array
{
...
{
    global $error_messages;
    $condition  = one($table, $value, $column) != false;
    $current_error_message = format_error_message($error_messages['exists'], "{attr}", $column);
    return  returnValidationResult($condition, $value, $current_error_message);
}
